...
def get_tts_params_for_chunk(chunk):
    ...
    if "tts_params" in chunk:
        ...
        print(
            f"📊 Using stored TTS params: exag={tts_params.get('exaggeration', 1.0)}, cfg={tts_params.get('cfg_weight', 0.7)}, temp={tts_params.get('temperature', 0.7)}"
        )
        return tts_params
    ...
    def get_float_input(prompt, default):
        """Prompts user for float input with an optional default value and validates the input as a number. Args: prompt (str): The input prompt message. default (float): The default value to use if the user does not provide any input. Returns: float: The validated float input from the user or the default value."""
        while True:
            try:
...
def synthesize_chunk(
    ...
    try:
        ...
        print(f"🔎 synthesize_chunk device: {device}")
        print(f"🔎 synthesize_chunk audio_dir: {audio_dir}")
        print(f"🔎 synthesize_chunk chunks_json_path: {chunks_json_path}")
        print(f"🔎 synthesize_chunk override_voice_name: {override_voice_name}")
        print(f"🔎 synthesize_chunk override_voice_path: {override_voice_path}")

        # Load TTS model
        print("🤖 Loading TTS model for chunk synthesis...")
        ...
        if override_voice_name:
            ...
            voice_path = override_voice_path or find_voice_file_by_name(
                override_voice_name
            )
            voice_name = override_voice_name
            detection_method = "user_selected"
            print(f"🔎 Resolved user-selected voice_path: {voice_path}")
        else:
            ...
            voice_name, voice_path, detection_method = detect_voice_for_book(
                book_name, chunks_json_path
            )

        # Fallback to first available voice if detection failed
        if not voice_path:
            print("⚠️ Voice not found, using first available voice")
            voice_files = list_voice_samples()
            if not voice_files:
        ...
        compatible_voice = ensure_voice_sample_compatibility(
            voice_path, output_dir=Path(audio_dir).parent
        )
        print(f"🔎 Compatible voice path: {compatible_voice}")
        ...
        print(
            f"📊 TTS params: exag={tts_params['exaggeration']}, cfg={tts_params['cfg_weight']}, temp={tts_params['temperature']}"
        )
        ...
        audio_segment, is_quarantined = smart_audio_validation_memory(
            audio_segment, model.sr
        )
        ...
        if boundary_type and boundary_type != "none":
            audio_segment = process_audio_with_trimming_and_silence(
