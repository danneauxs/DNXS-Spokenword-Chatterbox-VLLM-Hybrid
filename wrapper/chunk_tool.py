...
def select_book_for_repair():
    ...
    while True:
        try:
            choice = input(
                f"\nSelect book index [0-{len(available_books)-1}]: "
            ).strip()
            idx = int(choice)
            if 0 <= idx < len(available_books):
                ...
                print(
                    f"❌ Please enter a number between 0 and {len(available_books)-1}"
                )
...
def run_chunk_repair_tool():
    ...
    while True:
        ...
        while True:
            ...
            print(
                f"  🎛️  TTS Params: exag={tts_params.get('exaggeration', 'N/A')}, cfg={tts_params.get('cfg_weight', 'N/A')}, temp={tts_params.get('temperature', 'N/A')}"
            )
            ...
            if choice == "1":
                ...
                boundary = input(
                    "New boundary type (none/paragraph_end/chapter_start/chapter_end/section_break) [Enter to skip]: "
                ).strip()

                current_sentiment = chunk.get(
                    "sentiment_compound", chunk.get("sentiment_score", "N/A")
                )
                print(f"Current sentiment score: {current_sentiment}")
                sentiment = input(
                    "New sentiment compound score (-1.0 to 1.0) [Enter to skip]: "
                ).strip()
                ...
                def get_float_input(
                    param_name, current_val, min_val=None, max_val=None
                ):
                    """Prompt user for a floating-point input within specified range.
                    Args:
                    param_name (str): Name of the parameter being input.
                    current_val (float): Current value of the parameter.
                    min_val (float, optional): Minimum allowed value.
                    max_val (float, optional): Maximum allowed value.
                    Returns:
                    float: Validated floating-point input.
                    """
                    while True:
                        try:
                ...
                save_chunks(str(chunk_path), chunks)
                print(
                    f"✅ TTS parameters updated: exag={new_exag}, cfg={new_cfg}, temp={new_temp}"
                )
            elif choice == "5":
                print(f"\n🎤 Resynthesizing chunk {index+1:05d}...")
                revised_path = synthesize_chunk(
                    chunk, index, book_name, book_audio_dir, revision=True
                )
                if revised_path:
                ...
                print(
                    f"❌ Invalid option '{choice}'. Please enter a number 1-8 (or 'q' to quit)."
                )
