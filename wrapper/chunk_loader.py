...
def load_chunks(path):
    """Load chunks from a JSON file, filtering out metadata entries.
    Args:
    path: str - Path to the JSON file.
    Returns:
    list or dict - Chunks of data excluding metadata.
    """
    with open(path, "r", encoding="utf-8") as f:
        ...
        if isinstance(data, list):
            chunks = [
                item
                for item in data
                if not (isinstance(item, dict) and item.get("_metadata", False))
            ]
            return chunks
...
def save_chunks(path, chunks):
    """Saves chunks of data to a file after cleaning and validating them.
    Args:
    ...
    for chunk in chunks:
        if isinstance(chunk, dict) and "text" in chunk:
            ...
            if (
                "replied" in cleaned_text or "said" in cleaned_text
            ) and '"' in cleaned_text:
                # Additional cleanup for dialogue
                import re

                cleaned_text = re.sub(
                    r'(["\'])\s*,\s*(["\'])\s*\.', r"\1.", cleaned_text
                )  # Fix ", ". pattern
                cleaned_text = re.sub(
                    r'(["\'])\s*,\s*(["\'])\s*$', r"\1.", cleaned_text
                )  # Fix trailing ", "
                ...
                if cleaned_text != original_text:
                    print("🔧 FIXED dialogue corruption:")
