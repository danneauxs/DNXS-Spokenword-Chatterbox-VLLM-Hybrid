...
def _cangjie_mapping_path() -> Path | None:
    """Return local Cangjie mapping path when runtime already has file on disk.

    The portable launch path keeps multilingual sidecars under
    `CHATTERBOX_MTL_CKPT_DIR`, and the repo also carries a checked-in copy next
    to this tokenizer. Prefer those local sources so normal startup does not
    probe Hugging Face on every run.
    """
    candidates = []
    env_dir = os.environ.get("CHATTERBOX_MTL_CKPT_DIR")
    if env_dir:
        candidates.append(Path(env_dir).expanduser() / "Cangjie5_TC.json")
    candidates.append(Path(__file__).with_name("Cangjie5_TC.json"))
    candidates.append(
        Path(__file__).resolve().parents[4] / "models" / "chatterbox-mtl-v3" / "Cangjie5_TC.json"
    )
...
class ChineseCangjieConverter:
    ...
    def __init__(self):
        """Initializes an instance for Cangjie mapping and segmenter setup."""
        self.word2cj = {}
        self.cj2word = {}
        self.segmenter = None
    ...
    def _load_cangjie_mapping(self):
        ...
        try:
            cangjie_file = _cangjie_mapping_path()
            if cangjie_file is None:
                cangjie_file = Path(
                    hf_hub_download(repo_id=REPO_ID, filename="Cangjie5_TC.json")
                )
...
class MTLTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Initializes an instance with a tokenizer and sets up special tokens like UNK, PAD, SEP, etc."""
    ...
    def check_vocabset_sot_eot(self):
        """Checks if Start-Of-Text (SOT) and End-Of-Token (EOT) are in the tokenizer's vocabulary."""
        voc = self.tokenizer.get_vocab()
        assert SOT in voc
        assert EOT in voc

    def get_vocab(self):
        """Returns the tokenizer's vocabulary and includes a method to preprocess text."""
    ...
    def preprocess_text(self, raw_text: str, language_id: str = None, lowercase: bool = True, nfkd_normalize: bool = True):
        ...
        if lowercase:
            ...
            # Restore special-token spelling after lowercase. V3 T3 otherwise
            # treats "[start]"/"[stop]" as words and speaks them.
            for special in (SOT, EOT, UNK, SPACE):
                preprocessed_text = preprocessed_text.replace(special.lower(), special)
        if nfkd_normalize:
    ...
    def _tokenize(self, text: str, **kwargs) -> List[str]:        
        """Tokenizes text into tokens, handling language-specific processing like Cangjie for Chinese and Hiragana normalization for Japanese."""
    ...
    def _convert_token_to_id(self, token: str) -> int:
        """Converts a token to its corresponding ID using the tokenizer and includes helper methods."""
        return self.tokenizer.token_to_id(token)

    def _convert_id_to_token(self, index: int) -> str:
        """Converts an ID to its corresponding token using the tokenizer."""
        return self.tokenizer.id_to_token(index)

    def convert_tokens_to_string(self, tokens: List[str]) -> str:
        """Converts tokens to a string after cleaning them by removing unnecessary characters."""
        text = "".join(tokens)
        text = text.replace(' ', '')
        text = text.replace(SPACE, ' ')
    ...
    def vocab_size(self) -> int:
        """Returns the size of the tokenizer's vocabulary and includes a max_token_id property."""
        return self.tokenizer.get_vocab_size()
    
    @property
    def max_token_id(self) -> int:
        """Returns the maximum token ID from the tokenizer's vocabulary."""
