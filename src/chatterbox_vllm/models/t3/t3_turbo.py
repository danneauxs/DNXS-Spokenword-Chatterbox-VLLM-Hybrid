...
# GPT2 vocab is 50276, so prefill markers and speech-token offset must sit above it.
PREFILL_COND_START_TOKEN = 50276
PREFILL_COND_END_TOKEN = 50277
PREFILL_END_TOKEN = 50278

# 1 speaker token + 375 raw speech-cond tokens (Turbo has no perceiver).
CONDITIONING_SIZE = 376
SPEECH_TOKEN_OFFSET = 51000
TURBO_TEXT_VOCAB = 50276
TURBO_SPEECH_VOCAB = 6563


class T3ProcessingInfo(BaseProcessingInfo):
    """T3ProcessingInfo provides details about supported multi-modal limits for a specific processing task.
    T3MultiModalDummyInputsBuilder generates dummy inputs for testing multi-modal models, including textual and conditional data.
    T3MultiModalDataParser converts raw multi-modal data into structured items for processing.
    """
    def get_supported_mm_limits(self) -> Mapping[str, Optional[int]]:
        """Supported multi-modal limits as dictionary."""
        return {"conditionals": 1}


class T3MultiModalDummyInputsBuilder(BaseDummyInputsBuilder):
    """Dummy inputs builder for T3 model."""
    def get_dummy_text(self, mm_counts: Mapping[str, int]) -> str:
        """Get dummy text for testing."""
        return "Hello, world!"
...
class T3MultiModalDataParser(MultiModalDataParser):
    """Multi-modal data parser class."""
    def parse_mm_data(self, mm_data: MultiModalDataDict) -> MultiModalDataItems:
        """Parse multi-modal data to extract conditionals."""
...
class ConditionalsEmbeddingItems(ModalityDataItems[torch.Tensor, torch.Tensor]):
    """Conditionals embedding class for modality data."""
    def __init__(self, data: torch.Tensor) -> None:
        """Initialize with data and set modality type."""
        super().__init__(data, "conditionals")

    def get_count(self) -> int:
        """Returns a count value of 1."""
        """Return count as 1."""
        return 1
    ...
    def get_processor_data(self) -> Mapping[str, torch.Tensor]:
        """Returns processor-specific data, typically empty."""
        return {}

    def get_passthrough_data(self) -> Mapping[str, torch.Tensor]:
        """Gets passthrough data containing conditional information."""
        return {"conditionals": self.data}


def create_triangular_matrix(m, n):
    """Creates a triangular matrix for masking purposes."""
...
class T3MultiModalProcessor(BaseMultiModalProcessor[T3ProcessingInfo]):
    """Processes multimodal data using T3MultiModalDataParser."""
    def _get_data_parser(self) -> MultiModalDataParser:
        """Provides a data parser for multimodal data."""
    ...
    def _get_mm_fields_config(
        ...
        """Retrieves multimodal fields configuration based on HF inputs."""
        return dict(
    ...
    def _get_prompt_updates(
        ...
        """Returns a sequence of prompt updates."""
    ...
    def _call_hf_processor(
        ...
        """Calls HF processor with prompt, multimodal data, and tokenizer kwargs."""
...
class T3TurboVllmModel(nn.Module, VllmModelForTextGeneration, SupportsMultiModal):
    """vLLM T3 for Turbo GPT2 weights (no CFG, no perceiver)."""

    def __init__(self, *, vllm_config: VllmConfig, prefix: str):
        """Build GPT2 T3 stack for t3_turbo_v1.safetensors."""
        super().__init__()
        hf = vllm_config.model_config.hf_config
        # vLLM GPT2 reads hidden_size / n_embd from this config.
        if getattr(hf, "hidden_size", None) in (None, 0):
            hf.hidden_size = getattr(hf, "n_embd", 1024)
        self.vllm_config = vllm_config
        self.cfg: ModelConfig = vllm_config.model_config

        self.tfmr = GPT2Model(vllm_config=vllm_config, prefix="")

        self.t3conf = T3Config()
        self.t3conf.speech_tokens_dict_size = TURBO_SPEECH_VOCAB
        self.t3conf.speech_cond_prompt_len = 375
        self.t3conf.use_perceiver_resampler = False
        self.t3conf.emotion_adv = False
        self.t3conf.n_channels = 1024
        self.dim = 1024
        self.cond_enc = T3CondEnc(self.t3conf)
        self.text_emb = nn.Embedding(TURBO_TEXT_VOCAB, self.dim)
        self.speech_emb = nn.Embedding(TURBO_SPEECH_VOCAB, self.dim)
        self.speech_head = ParallelLMHead(
            num_embeddings=TURBO_SPEECH_VOCAB,
            embedding_dim=self.dim,
            bias=True,
            padding_size=1,
            prefix=prefix + ".speech_head",
        )
        self.logits_processor = LogitsProcessor(TURBO_SPEECH_VOCAB)
        print("Turbo T3 vLLM: CFG disabled (Turbo checkpoint has no CFG path)")


    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        """Loads weights into the model's state dictionary."""
        loaded_params: set[str] = set()
        state_dicts = {}
        gpt_weights = {}
        for name, weight in weights:
            if name.startswith("tfmr."):
                gpt_weights[name[5:]] = weight
                continue
        ...
        for attr, state_dict in state_dicts.items():
            if hasattr(self, attr):
                try:
                    ...
                    print(f"Turbo T3 skip/partial load {attr}: {exc}")

        gpt_loaded = self.tfmr.load_weights(gpt_weights.items())
        loaded_params.update('tfmr.' + i for i in gpt_loaded)
        return loaded_params

    def _embed_text(self, ids: torch.Tensor) -> torch.Tensor:
        """Look up GPT2 text embeddings, clamping ids so a split prefill cannot CUDA-abort."""
        if ids.numel() == 0:
            return ids.new_zeros((0, self.dim), dtype=self.text_emb.weight.dtype)
        safe = ids.clamp(0, TURBO_TEXT_VOCAB - 1)
        return self.text_emb(safe)

    def _embed_speech(self, ids: torch.Tensor) -> torch.Tensor:
        """Look up Turbo speech embeddings, clamping ids so OOB decode cannot CUDA-abort."""
        if ids.numel() == 0:
            return ids.new_zeros((0, self.dim), dtype=self.speech_emb.weight.dtype)
        safe = ids.clamp(0, TURBO_SPEECH_VOCAB - 1)
        return self.speech_emb(safe)


    def get_multimodal_embeddings(self, **kwargs: object) -> Optional[MultiModalEmbeddings]:
        """Gets multimodal embeddings from keyword arguments."""
    ...
    def get_input_embeddings(
        ...
        """Retrieves input embeddings based on input IDs."""
        if multimodal_embeddings is None or len(multimodal_embeddings) == 0:
            ...
            return self._embed_speech(input_ids - SPEECH_TOKEN_OFFSET)
            ...
            for ids, multimodal_embedding in self.split_prefill_decode(input_ids, multimodal_embeddings):
                ...
                if multimodal_embedding is None:
                    # There's no multimodal embeddings, so we're decoding.
                    # Remember to undo the offset we applied to the speech tokens.
                    embeds = self._embed_speech(ids - SPEECH_TOKEN_OFFSET)
                    final_embeds = embeds
                ...
                if ids[0] == PREFILL_COND_START_TOKEN and ids[-1] == PREFILL_END_TOKEN:
                    ...
                    text_emb = self._embed_text(text_ids)

                    start_of_speech_token = torch.tensor([self.t3conf.start_speech_token]).to(ids.device)
                    start_of_speech_emb = self._embed_speech(start_of_speech_token.unsqueeze(0))[0]
    ...
    def compute_logits(self, hidden_states: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        """Computes logits from hidden states and sampling metadata."""
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        # GPT2 wpe is 8196 long; clamp so a long prefill+decode cannot OOB.
        max_pos = int(self.tfmr.wpe.num_embeddings) - 1
    ...
    def get_language_model(self) -> torch.nn.Module:
        """Returns the language model module."""
        return self.tfmr