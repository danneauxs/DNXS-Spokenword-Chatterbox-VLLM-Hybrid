...
def drop_invalid_tokens(x):
    """Drop invalid tokens from input data, assuming batch size of one."""
...
def get_resampler(src_sr, dst_sr, device):
    """Function to get a resampler for converting audio between sample rates."""
...
class S3Token2Mel(torch.nn.Module):
    ...
    def __init__(self, use_fp16: bool = False):
        """Initialize the model with optional FP16 support, setting up various modules and encoders."""
        super().__init__()
    ...
    @property
    def device(self):
        """Retrieve the device on which the model's parameters are stored."""
        params = self.tokenizer.parameters()
        return next(params).device

    @property
    def dtype(self):
        """Get the data type of parameters in the model."""
        params = self.tokenizer.parameters()
    ...
    def embed_ref(
        ...
        """Embed reference waveform into a format suitable for processing."""
        ...
        # Speaker embedding - convert input to match speaker_encoder's dtype (FP16 fix)
        ref_wav_16_typed = ref_wav_16.to(next(self.speaker_encoder.parameters()).dtype)
        ref_x_vector = self.speaker_encoder.inference(ref_wav_16_typed)

        # Tokenize 16khz reference - tokenizer needs FP32
...
class S3Token2Wav(S3Token2Mel):
    ...
    def __init__(self, use_fp16: bool = False):
        """Initialize the model, setting up tokenizer, mel extractor, speaker encoder, and other components."""
    ...
    def forward(
        ...
        """Forward pass to compute output mel-spectrograms from input data."""
    ...
    # @torch.inference_mode()  # Commented for performance (enables TensorRT support)
    def flow_inference(
        self,
        speech_tokens,
        # locally-computed ref embedding (mutex with ref_dict)
        ...
        """Flow-based inference method for generating mel-spectrograms from speech tokens."""
        with torch.no_grad():  # Prevent gradient accumulation (OOM fix)
    ...
    def hift_inference(self, speech_feat, cache_source: torch.Tensor = None):
        """Shift inference for speech features with an optional cache source."""
        with torch.no_grad():  # Prevent gradient accumulation (OOM fix)
    ...
    def inference(
        ...
        """Perform inference using speech tokens and optional reference embeddings."""
        with torch.no_grad():  # Prevent gradient accumulation (OOM fix)
            ...
            # Convert mels to match mel2wav (HiFiGAN) dtype for FP16 compatibility
            output_mels = output_mels.to(next(self.mel2wav.parameters()).dtype)
