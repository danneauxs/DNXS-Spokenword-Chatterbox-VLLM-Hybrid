...
def _Gelu_no_versioncheck(self: GELU, gate: torch.Tensor) -> torch.Tensor:
    """Applies GELU activation function to a tensor.
    Args:
    gate (torch.Tensor): Input tensor to apply GELU activation on.
    Returns:
    torch.Tensor: Tensor after applying GELU activation.
    """
...
def _Gelu_forward_no_versioncheck(self: GELU, hidden_states):
    """Applies a GELU activation function to the input tensor.
    Args:
    hidden_states (torch.Tensor): Input tensor to apply GELU on.
    Returns:
    torch.Tensor: The transformed output tensor after applying GELU.
    ---
    Computes the forward pass for a linear layer with optional LoRA scaling.
    Args:
    hidden_states (torch.Tensor): Input tensor.
    scale (float, optional): Scaling factor for LoRA component. Default is 1.0.
    Returns:
    torch.Tensor: Output tensor from the linear transformation, possibly augmented by LoRA.
    """
...
def linear_forward_no_super(self: lora.LoRACompatibleLinear, hidden_states: torch.Tensor, scale: float = 1.0):
    """Defines a linear forward method for LoRACompatibleLinear layers with optional scaling."""
...
def apply_monkey_patches():
    """Applies monkey patches to modify forward methods of GELU, Attention, and LoRACompatibleLinear."""
