...
def get_num_wins(
    ...
    """Computes the number of windows and the target number of frames needed to ensure minimum coverage.
    Args:
    n_frames (int): Total number of frames.
    step (int): Step size between windows.
    min_coverage (float): Minimum required coverage.
    hp (VoiceEncConfig): Voice encoding configuration.
    Returns:
...
def get_frame_step(
    ...
    """Computes how many frames separate two partial utterances.
    Args:
    overlap (float): The overlap ratio between segments.
    rate (float): The rate at which to split the audio.
    hp (VoiceEncConfig): Configuration for voice encoding.
    Returns:
    int: The number of frames separating two partial utterances.
    """
...
class VoiceEncoder(nn.Module):
    """A neural network module for encoding voice data into speaker embeddings using an LSTM layer followed by a projection to a fixed embedding size. The class includes options for flattening LSTM parameters and adjusting cosine similarity scaling."""
    def __init__(self, hp=VoiceEncConfig()):
        """Initialize the VoiceEmbedding module.
        Args:
        hp (VoiceEncConfig): Configuration object for voice encoding parameters.
        Returns:
        None
        """
        super().__init__()
    ...
    def device(self):
        ...
        Args:
        mels (torch.FloatTensor): a batch of unscaled mel spectrograms
        Returns:
        torch.device: the device on which the model parameters are stored
        """
