...
def cast_all(*args, dtype):
    """Concats all inputs along a new dimension.
    Args:
    *args: Variable length argument list of arrays.
    dtype: Data type to cast non-float tensors to.
    Returns:
    List of input tensors with non-float types cast to the specified dtype.
    """
    return [a if (not a.dtype.is_floating_point) or a.dtype == dtype else a.to(dtype) for a in args]


class ConditionalCFM(BASECFM):
    """A conditional CFM model that extends BASECFM with speaker embedding for multi-speaker scenarios. Initializes with parameters for time scheduler and configuration rates for training and inference. Optionally accepts an estimator module to modify its architecture."""
    def __init__(self, in_channels, cfm_params, n_spks=1, spk_emb_dim=64, estimator: torch.nn.Module = None):
        """Initializes a model with specified parameters and an optional estimator.
        Args:
        in_channels (int): Number of input channels.
        cfm_params (object): Configuration parameters for the model.
        n_spks (int, optional): Number of speakers; default is 1.
        spk_emb_dim (int, optional): Dimension of speaker embeddings; default is 64.
        estimator (torch.nn.Module, optional): Pre-trained estimator module.
        Returns:
        None
        """
        super().__init__(
...
class CausalConditionalCFM(ConditionalCFM):
    """```python
    Causal Conditional CFM class extending ConditionalCFM for causal conditioning in diffusion models.
    Handles forward diffusion process with conditional inputs and optional noise addition.
    ```
    """
    def __init__(self, in_channels=240, cfm_params=CFM_PARAMS, n_spks=1, spk_emb_dim=80, estimator=None):
        ...
        Args:
        in_channels (int): Number of input channels.
        cfm_params (dict): Parameters for conditional flow model.
        n_spks (int): Number of speakers.
        spk_emb_dim (int): Dimension of speaker embeddings.
        estimator: Estimator for the diffusion process.
        Returns:
        None
        """
    ...
    def basic_euler(self, x, t_span, mu, mask, spks, cond):
        """Euler method integration for solving differential equations.
        Args:
        x (Tensor): Initial state.
        t_span (Tensor): Time span over which to solve the equation.
        mu (Tensor): Parameters for the model.
        mask (Tensor): Mask for the input data.
        spks (Tensor): Spikes for the neural network.
        cond (Tensor): Conditional inputs.
        Returns:
        Tensor: Final state after integration.
        """
        in_dtype = x.dtype
        x, t_span, mu, mask, spks, cond = cast_all(x, t_span, mu, mask, spks, cond, dtype=self.estimator.dtype)
