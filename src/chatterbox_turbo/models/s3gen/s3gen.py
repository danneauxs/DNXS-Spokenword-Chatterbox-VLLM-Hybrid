...
def drop_invalid_tokens(x):
    """Crops invalid tokens from a tensor.
    Args:
    x (Tensor): Input tensor of speech tokens.
    Returns:
    Tensor: Tensor with only valid tokens.
    ```
    """
...
def get_resampler(src_sr, dst_sr, device):
    """Returns an instance of ta.transforms.Resample configured for resampling between source and destination sample rates on specified device.
    Args:
    src_sr (int): Source sample rate.
    dst_sr (int): Destination sample rate.
    device (torch.device): Device to run the transformation on.
    """
...
class S3Token2Mel(torch.nn.Module):
    ...
    def __init__(self, meanflow=False):
        """Initializes a SpeechToText model with optional mean flow feature extraction.
        Args:
        meanflow (bool): Whether to enable mean flow in the model.
        Returns:
        None
        """
        super().__init__()
    ...
    def device(self):
        ...
        Args:
        device (str): The device to return. Default is "auto".
        Returns:
        str: The device of the first parameter.
        """
    ...
    def dtype(self):
        ...
        Args:
        ref_wav (torch.Tensor): The reference waveform tensor.
        ref_sr (int): The sample rate of the reference audio.
        device (str, optional): The device to use for processing. Defaults to "auto".
        ref_fade_out (bool, optional): Whether to apply fade out to the reference audio. Defaults to True.
        Returns:
        torch.dtype: The data type of the first parameter in the flow.
        """
    ...
    def embed_ref(
        ...
        Args:
        ref_wav (torch.Tensor): The input audio waveform.
        ref_sr (int): The sample rate of the reference audio.
        device (str, optional): Device to which the tensor will be moved. Defaults to "auto".
        ref_fade_out (bool, optional): Whether to apply a fade-out effect at the end of the audio.
        Returns:
        torch.Tensor: Embedded reference waveform.
        """
    ...
    def forward(
        ...
        - This function accepts a batch of token sequences. Each sequence may
          have a different length when speech_token_lens is supplied.
...
class S3Token2Wav(S3Token2Mel):
    ...
    def __init__(self, meanflow=False):
        """Initializes an instance with optional meanflow parameter. Sets up an F0 predictor and mel-to-wav generator for audio processing.
        Args:
        meanflow (bool): Whether to use mean flow in initialization.
        Returns:
        None
        """
    ...
    def flow_inference(
        ...
        """Performs flow inference on speech tokens.
        Args:
        speech_tokens (List[int]): The input speech tokens.
        ref_wav (Optional[torch.Tensor]): Locally-computed reference waveform embedding.
        ref_sr (Optional[int]): Sample rate of the reference waveform.
        ref_dict (Optional[dict]): Pre-computed reference embeddings dictionary.
        n_cfm_timesteps (int, optional): Number of confidence timesteps. Defaults to 2 if meanflow else 10.
        finalize (bool, optional): Whether to finalize the inference process. Defaults to False.
        speech_token_lens (List[int], optional): Lengths of individual speech tokens.
        Returns:
        None
        """
    ...
    def hift_inference(self, speech_feat, cache_source: torch.Tensor = None):
        ...
        cache_source (torch.Tensor, optional): Cache source tensor; defaults to an empty tensor if not provided.
        Returns:
    ...
    def inference(
        ...
        """Performs inference using speech tokens, optionally with reference audio embeddings for hallucination prevention.
        Args:
        - speech_tokens (list): List of speech tokens.
        - ref_wav (Optional[torch.Tensor]): Locally-computed reference waveform embedding.
        - ref_sr (Optional[int]): Reference waveform sampling rate.
        - ref_dict (Optional[dict]): Pre-computed reference embedding dictionary.
        - drop_invalid_tokens (bool, optional): Whether to drop invalid tokens. Defaults to True.
        - n_cfm_timesteps (Optional[int]): Number of confidence timesteps.
        - speech_token_lens (Optional[list]): List of speech token lengths.
        Returns:
