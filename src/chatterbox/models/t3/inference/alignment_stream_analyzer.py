...
class AlignmentAnalysisResult:
    """Represents the result of alignment analysis for a frame, indicating various conditions that may affect its quality and reliability within a text processing pipeline."""
...
class AlignmentStreamAnalyzer:
    """Class for analyzing alignment in transformer TTS models during streaming. Hooks into specified attention layer to perform integrity checks on alignment, repetition, etc., using heuristics based on self-attention activation maps. Current implementation requires no queues."""
    ...
    def _add_attention_spy(self, tfmr, alignment_layer_idx):
        ...
        def patched_forward(self, *args, **kwargs):
            """Applies a patch to the `forward` method of `target_layer` to enable attention outputs and then calls the original forward method.
            Args:
            *args: Positional arguments passed to the original forward method.
            **kwargs: Keyword arguments passed to the original forward method, with an additional 'output_attentions' set to True.
            Returns:
            The result of the patched `forward` method.
            """
    ...
    def step(self, logits):
        ...
        if aligned_attn is None or aligned_attn.numel() == 0:
            return logits
