...
class AttrDict(dict):
    """Class representing a Token-To-Token (T3) Text-to-Speech model utilizing Hugging Face transformer architectures as its backbone."""
...
def _ensure_BOT_EOT(text_tokens: Tensor, hp):
    """Ensures the presence of start and stop text tokens in each batch.
    Args:
    text_tokens (Tensor): The input tensor containing text tokens.
    hp: Hyperparameters containing start_text_token and stop_text_token.
    Raises:
    AssertionError: If any batch is missing start_text_token or stop_text_token.
    """
...
class T3(nn.Module):
    ...
    def __init__(self, hp=T3Config()):
        """Initializes a model with hyperparameters and sets up components for processing text and speech embeddings.
        Args:
    ...
    def device(self):
        ...
        Args:
        t3_cond: A T3Cond object containing conditioning data for speech embeddings and position embeddings.
        Returns:
        The device of the model's parameters.
        """
    ...
    def prepare_input_embeds(
        ...
        *,
        ...
        """Prepares input embeddings for T3 model by processing condition, text, and speech tokens.
        Args:
        t3_cond (T3Cond): The conditioning data.
        text_tokens (torch.LongTensor): Text token IDs.
        speech_tokens (torch.LongTensor): Speech token IDs.
        cfg_weight (float, optional): Weight for conditional generation. Default is 0.0.
        Returns:
        tuple: A tuple containing the condition embedding, text embedding, and speech embedding.
        """
    ...
    def forward(
        ...
        *,
        ...
        """Executes the forward pass for a model.
        Args:
        t3_cond (T3Cond): Conditional data.
        text_tokens (torch.LongTensor): Text input tokens.
        text_token_lens (torch.LongTensor): Lengths of text tokens.
        speech_tokens (torch.LongTensor): Speech input tokens.
    ...
    def loss(
        ...
        *,
        ...
        """Computes the training loss for a given set of tokens and their lengths.
        Args:
        t3_cond (T3Cond): The conditional input.
        text_tokens (torch.LongTensor): Text token tensor.
        text_token_lens (torch.LongTensor): Lengths of text tokens.
        speech_tokens (torch.LongTensor): Speech token tensor.
        speech_token_lens (torch.LongTensor): Lengths of speech tokens.
        Returns:
        None
        """
