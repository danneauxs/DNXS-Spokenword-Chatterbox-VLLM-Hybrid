...
def get_num_wins(
    ...
    """Calculate the number of windows and the target number of frames based on the given parameters.
    Args:
    n_frames (int): Total number of frames.
    step (int): Step size for window calculation.
    min_coverage (float): Minimum coverage required.
    hp (VoiceEncConfig): Configuration object containing partial frame information.
    Returns:
...
def get_frame_step(
    ...
    """Computes the number of frames separating two partial utterances.
    Args:
    overlap (float): The overlap ratio between two partial utterances.
    rate (float): The sampling rate for processing.
    hp (VoiceEncConfig): Configuration object containing frame-related settings.
    Returns:
...
class VoiceEncoder(nn.Module):
    """A neural network module for voice encoding that processes mel-spectrogram inputs and outputs speaker embeddings using an LSTM network followed by a linear projection layer. The model includes parameters for similarity scaling and bias to enhance embedding differences between speakers."""
    def __init__(self, hp=VoiceEncConfig()):
        """Initializes a VoiceEncoder instance.
        Args:
        hp (VoiceEncConfig): Configuration object for hyperparameters. Defaults to a new VoiceEncConfig instance if not provided.
        Returns: None
        """
        super().__init__()
    ...
    def device(self):
        ...
        Args:
        mels (torch.FloatTensor): Batch of unscaled mel spectrograms of shape (B, T, M).
        Returns:
