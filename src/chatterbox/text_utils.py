...
def split_by_word_boundary(
    text: str, max_len: int, lang: Optional[str] = None
) -> List[str]:
    """
...
def _split_cjk_text(text: str, max_len: int) -> List[str]:
    ...
    while i < len(text):
        ...
        if (
            len(current) > max_len * 0.8
            and re.search(phrase_boundaries, char)
            and i < len(text) - 1
        ):
            segments.append(current)
            current = ""
...
def split_text_into_segments(
    text: str, max_length: int = 300, logger: Optional[logging.Logger] = None
) -> List[str]:
    """
    ...
    if logger:
        logger.debug(
            f"Text length {len(text)} exceeds maximum limit {max_length}, starting split (language: {lang})"
        )
    ...
    for paragraph in paragraphs:
        ...
        sentences = merge_short_sentences(
            sentences, max_length, min_length=min_length, lang=lang
        )
    ...
    if logger:
        logger.debug(
            f"Text splitting completed, total {len(segments)} segments (language: {lang})"
        )
