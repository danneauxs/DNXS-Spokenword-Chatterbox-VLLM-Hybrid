...
def main():
    ...
    if not json_path.exists():
        ...
        print(
            "Please ensure you have run the 'Prepare text file' option for this book first."
        )
        return
    ...
    output_root, tts_dir, text_chunks_dir, audio_chunks_dir = setup_book_directories(
        Path(TEXT_INPUT_ROOT) / book_name
    )
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:  # Test parallel processing
        ...
        for i, chunk_data in enumerate(all_chunks):
            ...
            chunk_tts_params = {
                "exaggeration": chunk_data.get("tts_params", {}).get(
            ...
            future = executor.submit(
                ...
                model,
                None,
                chunk_data["is_paragraph_end"],
                all_chunks,
                chunk_data["boundary_type"],
            )
