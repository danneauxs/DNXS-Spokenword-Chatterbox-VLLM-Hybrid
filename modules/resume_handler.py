...
def analyze_existing_chunks(audio_chunks_dir):
    ...
    print("📊 Existing chunks analysis:")
    ...
    if missing_chunks:
        ...
        if len(missing_chunks) <= 10:
            ...
            print(
                f"   Missing: {missing_chunks[:10]}... (+{len(missing_chunks)-10} more)"
            )
...
def validate_resume_point(start_chunk, total_expected_chunks):
    ...
    if start_chunk > total_expected_chunks:
        print(
            f"{RED}❌ Resume point {start_chunk} exceeds expected total chunks {total_expected_chunks}{RESET}"
        )
        return False
...
def process_book_folder_resume(book_dir, voice_path, tts_params, device, start_chunk=1):
    ...
    if start_chunk == 1:
        print("⚠️ WARNING: start_chunk is 1 - this will clear existing chunks!")
    ...
    if json_file:
        ...
        print("💡 Use Option 1 to process this book from the beginning first.")
        return None, None, []
    ...
    print(
        f"📊 Processing {remaining_chunks} remaining chunks in batches of {BATCH_SIZE}"
    )
    ...
    for batch_start in range(0, remaining_chunks, BATCH_SIZE):
        ...
        if ENABLE_ASR:
            ...
            print("🎤 Loading ASR model for resume mode...")
        ...
        print(
            f"🔧 Using {optimal_workers} workers for batch {actual_start_chunk}-{actual_end_chunk}"
        )
        ...
        if ENABLE_PRODUCER_CONSUMER_PIPELINE:
            try:
                print(
                    f"🚀 Attempting producer-consumer pipeline for resume batch {actual_start_chunk}-{actual_end_chunk}"
                )
                ...
                pipeline_results = process_chunks_with_pipeline(
                    ...
                    asr_model,
                    True,
                    optimal_workers,  # asr_enabled=True for resume
                ...
                if batch_results:
                    print(
                        f"✅ Producer-consumer pipeline completed resume batch: {len(batch_results)} chunks"
                    )
        ...
        if not batch_results:
            print(
                f"🔄 Using sequential processing fallback for resume batch {actual_start_chunk}-{actual_end_chunk}"
            )
            futures = []
            ...
            with ThreadPoolExecutor(max_workers=optimal_workers) as executor:
                ...
                print(
                    f"🔄 {CYAN}Waiting for batch {actual_start_chunk}-{actual_end_chunk} to complete...{RESET}"
                )
                completed_count = 0
        ...
        print(
            f"✅ Batch {actual_start_chunk}-{actual_end_chunk} completed ({len(batch_results)} chunks)"
        )
...
def resume_book_from_chunk(start_chunk):
    ...
    book_dirs = sorted(
        [
            d
            for d in audiobook_root.iterdir()
            if d.is_dir() and d.name != "Audio_Revisions"
        ]
    )
    if not book_dirs:
        print(
            f"{RED}No books found in {AUDIOBOOK_ROOT}/ - no books have started processing.{RESET}"
        )
        print("💡 Use Option 1 to start processing a new book first.")
        return None
        ...
        try:
            ...
            if 0 <= book_idx < len(book_dirs):
                ...
                if text_input_book_dir.exists():
                    ...
                    print("💡 The original book files may have been moved or deleted.")
                    continue
                break
        except Exception:
            pass
    ...
    def prompt_float(prompt, default):
        """Prompts user to enter a float value with an optional default.
        Args:
        prompt (str): The question or instruction for the user.
        default (float): The default value if user input is empty.
        Returns:
        float: The entered or default float value.
        """
        val = input(f"{prompt} [{default}]: ").strip()
        return float(val) if val else default

    exaggeration = prompt_float(
        "Enter exaggeration (emotion intensity)", DEFAULT_EXAGGERATION
    )
    cfg_weight = prompt_float(
        "Enter cfg_weight (faithfulness to text)", DEFAULT_CFG_WEIGHT
    )
    ...
    # Process with resume
    return process_book_folder_resume(
        book_dir, voice_path, tts_params, device, start_chunk
    )
...
def auto_resume_incomplete():
    ...
    for i, book in enumerate(incomplete):
        print(
            f"  [{i}] {book['name']} (last chunk: {book['last_chunk']}, missing: {book['missing_chunks']})"
        )

    choice = input(
        f"\nSelect book to resume [0-{len(incomplete)-1}] or 'q' to quit: "
    ).strip()
