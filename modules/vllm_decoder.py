...
class VllmDecoder:
    ...
    def _init_standard(self, ckpt_dir: str | Path):
        """Initialize original S3Gen without creating an unrelated vLLM T3.

        Standard decode receives speech tokens already generated by Phase 1.
        It needs only the original S3Gen and voice conditionals.  Loading the
        vLLM wrapper here rebuilt a second T3 with its default English text
        embedding, which cannot load multilingual V2/V3 checkpoints.
        """
        from src.chatterbox.tts import ChatterboxTTS

        logger.info("Loading original standard S3Gen decoder from %s", ckpt_dir)
        model = ChatterboxTTS.from_local(
            ckpt_dir=ckpt_dir,
            device=self.target_device,
        )
        # Original S3Gen stores diffusion-step count on its flow module rather
        # than accepting vLLM S3Gen's ``n_timesteps`` inference keyword.
        model.set_diffusion_steps(self.diffusion_steps)
        self._model = model
        self._sample_rate = model.sr
        if voice_path := self.voice_path:
            model.prepare_conditionals(str(voice_path))
            self._s3gen_ref = model.conds.gen
        else:
            if model.conds is None:
                raise RuntimeError(
                    "Standard decoder needs a voice path when the checkpoint has no conds.pt"
                )
            self._s3gen_ref = model.conds.gen
    ...
    def decode_and_assemble_chunk(
        ...
        the now-trimmed segments, THEN any trailing explicit pause-only silence is
        appended, THEN contextual boundary silence is added once at the very end.
        ...
        trimmed_ms_total = 0
        pauses_inserted = []
        inter_segment_count = min(len(pauses), max(0, len(segments) - 1))
        inter_segment_pauses = pauses[:inter_segment_count]
        trailing_pause_pauses = pauses[inter_segment_count:]
        for seg_idx, seg_tokens in enumerate(segments):
            ...
            if combined is None:
                ...
                pause_idx = seg_idx - 1
                if pause_idx < len(inter_segment_pauses):
                    pause_ms = int(inter_segment_pauses[pause_idx] * 1000)
                    if pause_ms > 0:
                        combined += AudioSegment.silent(duration=pause_ms)
                        pauses_inserted.append(pause_ms)
                combined += seg

        trailing_pause_ms = sum(max(0.0, pause) for pause in trailing_pause_pauses)
        if trailing_pause_ms > 0:
            trailing_pause_ms = int(round(trailing_pause_ms * 1000))
            if combined is None:
                combined = AudioSegment.silent(duration=trailing_pause_ms)
            else:
                combined += AudioSegment.silent(duration=trailing_pause_ms)
            pauses_inserted.append(trailing_pause_ms)
