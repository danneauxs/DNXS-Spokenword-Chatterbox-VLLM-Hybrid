...
class TerminalLogger:
    ...
    def __init__(self, log_file="term.log", also_print=True):
        """Initializes a logger that optionally logs to a file and also prints to console.
        Args:
        log_file (str): Path to the log file. Defaults to "term.log".
        also_print (bool): Whether to print log messages to the console as well. Defaults to True.
        """
    ...
    def emit_chunk_summary(self):
        ...
        with self._sampling_lock:
            try:
                if (
                    self._last_sampling_tokens is None
                    or self._last_sampling_its is None
                ):
                    return  # nothing captured from Sampling yet
                self._emit_chunk_summary(
                    self._last_sampling_tokens, self._last_sampling_its
                )
            finally:
                ...
                if clean_line.startswith("Sampling:"):
                    ...
                    try:
                        ...
                        if m_tokens:
                            ...
                            if self._last_sampling_tokens is None or current < (
                                self._last_sampling_tokens or 0
                            ):
                                self._last_sampling_tokens = None
                                self._last_sampling_total = None
                                self._last_sampling_its = None
    ...
    def set_eta_frequency(self, every: int):
        """Set the frequency of eta updates.
        Args:
        every (int): The number of iterations between eta updates. Must be a positive integer.
        Returns:
        None
        ---
        Set the batch size for sampling.
        Args:
        size (int): The size of each batch. Must be a non-negative integer.
        Returns:
        None
        """
        with self.lock:
    ...
    def set_batch_size(self, size: int):
        """Starts terminal logging to a specified file.
        Args:
        log_file (str): The name of the log file. Default is "term.log".
        """
        with self._sampling_lock:
            try:
...
def log_only(text: str):
    ...
    if _terminal_logger is not None:
        ...
        try:
            Path("term.log").open("a", encoding="utf-8").write(
...
def set_eta_frequency(every: int):
    """Sets the ETA frequency for the terminal logger.
    Args: every (int): The frequency at which to update the ETA.
    Sets the batch size for the terminal logger.
    Args: size (int): The new batch size.
    Emits a summary chunk using the terminal logger.
    """
    global _terminal_logger
    if _terminal_logger is not None:
...
def set_batch_size(size: int):
    """Sets the batch size for logging.
    Args:
    size (int): The new batch size.
    Returns:
    None
    Emits a summary of the current chunk data to the logger.
    Does not take any arguments.
    Returns:
    None
    Retrieves the running average iterations per second from the terminal logger.
    """
    global _terminal_logger
    if _terminal_logger is not None:
...
def emit_chunk_summary():
    """emit_chunk_summary(): Emits a chunk summary using the global terminal logger if it's not None.
    Args: None
    Returns: None
    get_running_avg_its(): Retrieves the running average iterations per second from per-chunk summaries using the global terminal logger if it's not None.
    Args: None
    Returns: Running average it/s as a float, or None if an exception occurs or the logger is not initialized.
    """
    global _terminal_logger
    if _terminal_logger is not None:
