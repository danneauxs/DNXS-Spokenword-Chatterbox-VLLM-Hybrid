...
def list_voice_samples():
    """List supported voice sample files from configured voice directory."""
    print(f"🔎 list_voice_samples file: {__file__}")
    print(f"🔎 list_voice_samples cwd: {os.getcwd()}")
    print(f"🔎 list_voice_samples sys.path[0:5]: {sys.path[:5]}")
    print(f"🔎 list_voice_samples dir: {VOICE_SAMPLES_DIR}")
    if not VOICE_SAMPLES_DIR.is_dir():
        print("⚠️ Voice_Samples dir missing")
        return []
    ...
    for voice_file in voice_files:
        print(f"🔎 Voice sample candidate: {voice_file.name}")
    print(f"🔎 Total voice samples found: {len(voice_files)}")
    return voice_files


def ensure_voice_sample_compatibility(input_path, output_dir=None):
    """Create and return a 24 kHz mono copy for every voice input."""
    input_path = Path(input_path)
    output_dir = Path(output_dir) if output_dir else input_path.parent
    output_name = input_path.stem
    if not output_name.endswith("_ttsready"):
        output_name += "_ttsready"
    output_path = output_dir / f"{output_name}.wav"

    print(f"🔎 ensure_voice_sample_compatibility input: {input_path}")
    print(f"🔎 ensure_voice_sample_compatibility output_dir: {output_dir}")
    print(f"🔎 ensure_voice_sample_compatibility output_path: {output_path}")

    if input_path.resolve() == output_path.resolve():
        print("🔎 Input equals output path; returning without conversion")
        return str(output_path)

    output_dir.mkdir(parents=True, exist_ok=True)
    print(f"🔎 Running ffmpeg normalization for {input_path.name}")
    cmd = [
        "ffmpeg",
        "-y",
        "-i",
    ...
    print(f"🔎 Wrote compatible voice: {output_path}")
    return str(output_path)
...
def run_ffmpeg(cmd):
    ...
    try:
        ...
        if e.returncode == 254:
            logging.error(
                "FFmpeg exit code 254 - Usually indicates file I/O or format issues"
            )
...
def convert_to_m4b_with_peak_normalization(
    wav_path, temp_m4b_path, target_db=-3.0, custom_speed=None, custom_sample_rate=None
):
    ...
    sample_rate_to_use = (
        custom_sample_rate if custom_sample_rate is not None else M4B_SAMPLE_RATE
    )
    ...
    for line in process.stderr:
        ...
        if match:
            ...
            print(
                f"📼 FFmpeg (normalizing): {match.group(0)} | {factor:.2f}x realtime",
                end="\r",
            )
...
def convert_to_m4b_with_loudness_normalization(
    wav_path, temp_m4b_path, custom_speed=None, custom_sample_rate=None
):
...
def convert_to_m4b_with_simple_normalization(
    wav_path, temp_m4b_path, target_db=-6.0, custom_speed=None, custom_sample_rate=None
):
...
def convert_to_m4b(wav_path, temp_m4b_path, custom_speed=None, custom_sample_rate=None):
    ...
    if not ENABLE_NORMALIZATION or NORMALIZATION_TYPE == "none":
        ...
        cmd = (
            ...
            + audio_filter
            + ["-ar", str(sample_rate_to_use), "-c:a", "aac", str(temp_m4b_path)]
        )

    elif NORMALIZATION_TYPE == "loudness":
        # EBU R128 loudness normalization (recommended for audiobooks)
        return convert_to_m4b_with_loudness_normalization(
            wav_path, temp_m4b_path, custom_speed, custom_sample_rate
        )

    elif NORMALIZATION_TYPE == "peak":
        # Peak normalization
        return convert_to_m4b_with_peak_normalization(
            wav_path, temp_m4b_path, TARGET_PEAK_DB, custom_speed, custom_sample_rate
        )

    elif NORMALIZATION_TYPE == "simple":
        # Simple volume adjustment
        return convert_to_m4b_with_simple_normalization(
            wav_path, temp_m4b_path, TARGET_PEAK_DB, custom_speed, custom_sample_rate
        )
...
def find_book_files(book_dir):
    ...
    cover_file = (
        cover_jpg if cover_jpg.exists() else cover_png if cover_png.exists() else None
    )
...
def apply_batch_binning(chunks_info):
    ...
    print(
        f"📦 BATCH-BINNING: Processing {len(chunks_info)} chunks with precision {BATCH_BIN_PRECISION}"
    )
    binned_chunks = []
    ...
    if total_changes > 0:
        ...
        for param, changes in param_changes.items():
            if changes:
                ...
                print(
                    f"   {param}: {len(changes)} chunks changed, {unique_original}→{unique_binned} unique values"
                )
        ...
        for chunk in chunks_info:
            if "tts_params" in chunk and chunk["tts_params"]:
                tp = chunk["tts_params"]
                orig_combo = (
                    tp.get("exaggeration", 0.5),
                    tp.get("cfg_weight", 0.5),
                    tp.get("temperature", 0.85),
                )
        ...
        print(
            f"📦 MICRO-BATCH IMPROVEMENT: {len(original_combinations)}→{len(binned_combinations)} parameter combinations"
        )
        improvement = (
            (len(original_combinations) - len(binned_combinations))
        ...
        print(
            f"📦 Batch consolidation: {improvement:.1f}% reduction in unique parameter sets"
        )
    else:
