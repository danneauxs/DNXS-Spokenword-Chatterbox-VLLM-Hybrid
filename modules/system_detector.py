...
def categorize_system(profile):
    ...
    return {"vram": vram_category, "ram": ram_category, "cpu": cpu_category}
...
def recommend_asr_models(profile):
    ...
    recommendations["safe"] = {
        "primary": {
            "model": safe_gpu or safe_cpu,
            "device": "gpu" if safe_gpu else "cpu",
        },
    ...
    recommendations["moderate"] = {
        "primary": {
            "model": moderate_gpu or moderate_cpu,
            "device": "gpu" if moderate_gpu else "cpu",
        },
    ...
    recommendations["insane"] = {
        "primary": {
            "model": insane_gpu or insane_cpu,
            "device": "gpu" if insane_gpu else "cpu",
        },
...
def print_system_summary(profile):
    ...
    print(
        f"   VRAM: {profile['gpu']['total_mb']:,}MB total, {profile['available_vram_after_tts']:,}MB available after TTS ({categories['vram']} class)"
    )
    print(
        f"   RAM:  {profile['ram']['total_mb']:,}MB total, {profile['ram']['available_mb']:,}MB available ({categories['ram']} class)"
    )
    print(f"   CPU:  {profile['cpu_cores']} cores ({categories['cpu']} class)")

    if not profile["has_gpu"]:
        print("   ⚠️ No CUDA GPU detected - ASR will run on CPU only")
...
if __name__ == "__main__":
    ...
    for level, config in recommendations.items():
        ...
        print(
            f"🟢 {level.upper()}: {primary['model']} ({primary['device']}) + {fallback['model']} (cpu fallback)"
        )
